...
export function WarehousePage() {
  ...
  async function fetchStock() {
    try {
      ...
      const movements =
        (await blink.db.movimenti_magazzino.list()) as MovimentoMagazzino[];

      // 3. Calculate current quantity for each
      const calculated = ingredients.map((ing) => {
        const ingMovements = movements.filter(
          (m) => m.ingredienteId === ing.id,
        );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Monitora le scorte e gestisci i carichi di merce.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 gap-4">
          {filteredStock.map((item) => (
            <Card
              key={item.id}
              className="border-none shadow-elegant overflow-hidden"
            >
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Scorta minima: {item.scortaMinima} {item.unit}
                        </p>
                      </div>
                    </div>
                  </div>
