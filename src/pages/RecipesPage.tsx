    ...
    try {
      ...
      for (const recipe of data) {
        ...
        const totalWithOverhead =
          recipeTotal * (1 + recipe.overheadPercent / 100);
        costs[recipe.id] =
          recipe.resaBatch > 0 ? totalWithOverhead / recipe.resaBatch : 0;
      }
      setFoodCosts(costs);
    } catch {
    } finally {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Crea ricette e calcola il costo di produzione.
          </p>
        </div>
        <Button
          onClick={() => {
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredRecipes.map((recipe) => (
            <Card
              key={recipe.id}
              className="border-none shadow-elegant overflow-hidden group"
            >
                ...
                <div className="p-6 space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Resa: {recipe.resaBatch} {recipe.unitaResa}
                    </p>
                  </div>
                  ...
                  <div className="pt-4 border-t flex items-center justify-between">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span className="text-primary font-bold italic">
                        {foodCosts[recipe.id]
                          ? `€ ${foodCosts[recipe.id].toFixed(2)} / ${recipe.unitaResa}`
                          : "N/D"}
                      </span>
                    </div>
                    <Button
